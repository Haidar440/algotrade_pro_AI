...
import { AnalysisResult, LoadingStep, PaperTrade, View, BrokerState, SignalFeedItem, MarketIndices } from './types';
import { INDIAN_STOCKS, Stock } from './services/stockData';
...
import { 
  Activity, Search, AlertTriangle, Moon, Filter, RefreshCw, Lock, Zap
} from 'lucide-react';
...
const App: React.FC = () => {
  ...
  const steps: LoadingStep[] = [
   ...
   const handleSessionUpdate = (newTokens: any) => {
    console.log("🔄 App: Updating Session Tokens...", newTokens);
    setBrokerState(prev => {
       const newState = { 
          ...prev, 
          angel: { ...prev.angel, ...newTokens } 
       };
      ...
      for (const stock of stocksToScan) {
        try {
          ...
          if (history && history.length > 50) {
             ...
             if (rec.signal === 'BUY') {
                finalSignal = rec.confidence > 0.8 ? 'STRONG BUY' : 'BUY';
          ...
          console.warn(`Skipping ${stock.symbol}`, e);
        }
  ...
  // --- EFFECTS ---
// --- REAL-TIME PRICE UPDATES (API + WEBSOCKET) ---
  useEffect(() => {
    ...
    const startLiveFeed = async () => {
       try {
          ...
          console.log(`🔌 Connecting for ${result.symbol} (Token: ${token})`);
          
          // ✅ 1. FETCH PRICE IMMEDIATELY (Fixes 0.00 on Weekends)
          // We assume "getLtpValue" is public in angel.ts or we call it via proxy
          try {
             const ltpResponse = await angel.getLtpValue("NSE", token, result.symbol); // Make sure getLtpValue is public in angel.ts
          ...
          } catch (err) { console.warn("Could not fetch initial LTP"); }

          // 2. Subscribe to Socket for Live Updates (When market opens)
    ...
    return () => {
       if (activeToken) streamer.unsubscribe(activeToken);
    };
 ...
 // --- REAL-TIME PRICE UPDATES (NEW WEBSOCKET VERSION) ---
  useEffect(() => {
    // Only run if we have a stock result and are connected to Angel One
    if (!result || !brokerState.angel) return;

    let activeToken: string | null = null; // Store token to unsubscribe later

    const startLiveFeed = async () => {
       try {
          const angel = new AngelOne(brokerState.angel);
          
          // 1. Get the correct Token ID (e.g. "3045" for SBIN)
          // The WebSocket NEEDS the number, not the name.
          const token = await angel.searchSymbolToken(result.symbol);
          activeToken = token;

          console.log(`🔌 Connecting Live Stream for ${result.symbol} (Token: ${token})`);
          
          // 2. Subscribe using the TOKEN
          streamer.subscribe(token, (newPrice) => {
             // Update the big number on the screen
             setLivePrice(newPrice);
             
             // Update the result object so P&L/Indicators use the new price
             setResult(prev => prev ? { ...prev, current_price: newPrice } : null);
          });
    ...
    // Cleanup: Stop listening when we leave this stock or switch to another
    return () => {
       if (activeToken) {
           streamer.unsubscribe(activeToken);
       }
    };
  }, [result, brokerState.angel]);
  ...
  // --- HANDLERS ---
  // --- SEARCH HANDLER (Fixed to use Database) ---
  const handleInputChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (value.length >= 2) {
      try {
        // ✅ 1. Try to search using Angel One Service (Local DB)
        if (brokerState.angel) {
           const angel = new AngelOne(brokerState.angel);
        ...
        console.warn("DB Search failed, falling back to static list");
      }

      // ✅ 2. Fallback: If DB fails or no broker, use static list
  ...
  const runAnalysis = async (symbol: string) => {
    ...
    try {
      if (!brokerState.angel) throw new Error("Please connect your Angel One account in Settings.");
      const angel = new AngelOne(brokerState.angel, handleSessionUpdate);
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setShowSuggestions(false);
    runAnalysis(ticker.toUpperCase());
  };

  const handleRefresh = () => {
    if (result) runAnalysis(result.symbol);
    else if (ticker) runAnalysis(ticker.toUpperCase());
  };

 const handleManualPriceUpdate = (newPrice: number) => {
     setLivePrice(newPrice);
     // We do NOT call streamer.subscribe here anymore.
     // The main useEffect handles the connection automatically.
     if (result) {
         setResult({ ...result, current_price: newPrice });
     }
  };
  ...
  const addToWatchlist = (signal: SignalFeedItem) => {
    if (!watchlist.find(i => i.id === signal.id)) {
      setWatchlist(prev => [signal, ...prev]);
    }
  };
       ...
       <div className="space-y-6 mt-6">
         <div className="flex flex-col gap-3">
            <div className="flex justify-between items-end">
               <div className="flex items-center gap-2 text-slate-400 text-sm font-bold uppercase tracking-wider">
                  <Filter className="w-4 h-4" /> Market Scanner
               </div>
               {brokerState.angel && (
         ...
         <div>
            <h3 className="text-white font-bold mb-4 flex items-center gap-2">
              <Activity className="w-5 h-5 text-emerald-400" /> Featured Signals
            </h3>
            ...
            {brokerState.angel && filteredSignals.length === 0 && !scanning && (
               <div className="p-8 text-center text-slate-500">
                  No signals found matching "{filterType}". Try refreshing.
               </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {filteredSignals.map(sig => (
                 <SignalFeedCard 
                    ...
                    isInWatchlist={watchlist.some(w => w.id === sig.id)}
                 />
              ))}
            </div>
         </div>
  ...
  return (
    <div className="flex h-screen bg-[#0f172a] text-slate-200 font-sans selection:bg-emerald-500/30 overflow-hidden relative">
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <main className="flex-1 overflow-y-auto bg-slate-900/50 p-4 md:p-8 scroll-smooth pb-24 md:pb-8">
          {currentView === 'SCANNER' && (
            <div className="max-w-7xl mx-auto pb-20 md:pb-0 animate-in fade-in slide-in-from-bottom-2 duration-500">
              {!result && (
                <div className="mb-6 relative max-w-full group z-20">
                    <form onSubmit={handleSearch} className="relative">
                      <div className="relative flex items-center bg-[#1e293b] rounded-xl p-2 border border-slate-700 focus-within:border-emerald-500/50 transition-colors shadow-lg">
                        ...
                        <input 
                          ...
                          onFocus={() => { if(ticker.length > 0) setShowSuggestions(true); }}
                          placeholder="Search NSE/BSE Stock (e.g. RELIANCE)..." 
                        ...
                        <button 
                          type="submit" 
                          disabled={loading}
                          className="bg-emerald-500 hover:bg-emerald-600 text-white px-4 py-2 rounded-lg font-semibold text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed shrink-0"
                        >
                          {loading ? '...' : 'Go'}
                        </button>
                      </div>
                    ...
                    {showSuggestions && suggestions.length > 0 && (
                      <div ref={wrapperRef} className="absolute top-full left-0 right-0 mt-2 bg-[#1e293b] border border-slate-700 rounded-xl shadow-2xl overflow-hidden max-h-64 overflow-y-auto">
              ...
              {error && (
                <div className="p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl flex items-center gap-3 text-rose-200 mb-8">
                  <AlertTriangle className="w-6 h-6 shrink-0" />
                  <p>{error}</p>
                  <button onClick={() => setShowSettingsModal(true)} className="ml-auto bg-rose-500/20 hover:bg-rose-500/30 px-3 py-1 rounded text-xs font-bold">Settings</button>
                </div>
              )}
              ...
              {result && !loading && (
                <StockDetailView 
                   ...
                   onPaperTrade={executePaperTrade}
                   onPriceEdit={handleManualPriceUpdate}
                   isInWatchlist={watchlist.some(w => w.symbol === result.symbol)}
                   onToggleWatchlist={() => {
                      if (watchlist.some(w => w.symbol === result.symbol)) {
                         setWatchlist(prev => prev.filter(w => w.symbol !== result.symbol));
                      } else {
                         const newItem: SignalFeedItem = {
                            id: `man_${result.symbol}_${Date.now()}`,
                            symbol: result.symbol,
                            name: INDIAN_STOCKS.find(s => s.symbol === result.symbol)?.name || result.symbol,
                            price: livePrice, 
                            changePercent: 0,
                            signal: result.primary_recommendation.signal as any,
                            confidence: result.primary_recommendation.confidence,
                            strategy: result.primary_recommendation.strategy_name,
                            entry: result.primary_recommendation.ideal_entry_range?.[0] || result.current_price,
                            stopLoss: result.primary_recommendation.stop_loss || 0,
                            target: result.primary_recommendation.target_prices?.[0] || 0,
                            timeframe: 'Swing',
                            sector: 'Unknown',
                            timestamp: new Date().toISOString()
                         };
          ...
          {currentView === 'WATCHLIST' && (
             <div className="max-w-4xl mx-auto space-y-6">
                <div className="flex justify-between items-center mb-6">
                   ...
                   <button onClick={refreshWatchlist} className="p-2 bg-slate-800 rounded-lg hover:bg-slate-700 transition-colors" disabled={isRefreshingWatchlist}>
                      <RefreshCw className={`w-4 h-4 text-slate-400 ${isRefreshingWatchlist ? 'animate-spin' : ''}`} />
                   </button>
                </div>
          ...
          {currentView === 'PORTFOLIO' && (
            <div className="max-w-7xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
               ...
               {brokerState.angel?.jwtToken && (
                  <ExecutionDashboard brokerState={brokerState} />
               )}

               {/* PAPER TRADING */}
               <div className="pt-8 border-t border-slate-800">
                  <h2 className="text-xl font-bold text-slate-400 mb-4 flex items-center gap-2">
                     <Activity className="w-5 h-5" /> Paper Trading Simulator
                  </h2>
          ...
          {currentView === 'STRATEGIES' && <div className="max-w-7xl mx-auto"><StrategyGuide /></div>}

          {currentView === 'PYTHON_LAB' && <div className="max-w-7xl mx-auto h-full"><PythonLab data={result} brokerState={brokerState} /></div>}
        </main>
